...
treespath="/lustrefs/hdd_pool_dir/nanoAODv12/wz-run3/trees/mc/2022EE/btagEff/"
files = os.listdir(treespath)
...
for groupname, procs in groups.items():
    if not os.path.exists( os.path.join(outdir, groupname)):
        os.system(" mkdir -p %s"%(os.path.join(outdir, groupname) ))
    ...
    for rf in files_toAdd:
        tChain.Add( os.path.join(treespath, rf + "/Friends"))
   
   
    # Prepare output
    outfile = os.path.join( outdir, groupname, "btagEff_DeepJet_2022EE.root")
    ...
    for wp in WPs:
        ...
        for f in flavors.keys():
            ...
            subs_pt = {"wp" : wp, "fl": f, "var" : "pt" }
            subs_eta = {"wp" : wp, "fl": f, "var" : "eta" }
            ...
            hPass  = r.TH2F("hp_{wp}_{fl}_{var}".format(**subs_pt),"hp", 3, array.array('d',[0, 0.8, 1.6, 2.5]), 8, array.array('d', [0, 30, 50, 70, 100, 140, 200, 300, 600, 1000]))
            hFail = r.TH2F("hf_{wp}_{fl}_{var}".format(**subs_pt),"hf", 3, array.array('d',[0, 0.8, 1.6, 2.5]), 8, array.array('d', [0, 30, 50, 70, 100, 140, 200, 300, 600, 1000]))
            ...
            c.SaveAs("%s/%s_2022EE.pdf"%(os.path.join(outdir, groupname), name))
            c.SaveAs("%s/%s_2022EE.png"%(os.path.join(outdir, groupname), name))
            c.SaveAs("%s/%s_2022EE.root"%(os.path.join(outdir, groupname), name))