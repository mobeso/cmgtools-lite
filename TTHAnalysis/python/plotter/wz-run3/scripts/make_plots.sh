...
if [[ $mode == *analysis* ]]; then
    ...
    python wz-run3/wz-run.py plot $outname --plot-group main
    echo "-----------------------------"
    ...
    for flav in eee eem mme mmm; do
        ...
        python wz-run3/wz-run.py plot --extra "-E ^flav_${flav}" $outname --plot-group main
        echo "-----------------------------"
    done
    fi
fi
...
if [[ $mode == *trigger_eff* ]]; then
    ...
    for barrelN in Inclusive barrel0 barrel1 barrel2 barrel3; do 
        ...
        python wz-run3/wz-run.py plot --mca $mca --cutfile $cuts --extra "--uf -E ^trigger $extracut" $outname --plot-group trigger
        echo "-----------------------------"
        ...
        python wz-run3/wz-run.py plot --mca $mca --cutfile $cuts  $outname --plot-group trigger --extra " --uf $extracut "
        echo "-----------------------------"
    done
